...
from model.HFR_NO1 import Model
...
parser.add_argument('--save_name', type=str, default='HFR_ns_experiment')
...
def main():
    ...
    input_data = torch.tensor(data['input'], dtype=torch.float)
    output_data = torch.tensor(data['output'], dtype=torch.float).transpose(-2, -1)
    print(input_data.shape, output_data.shape)
    x_train = input_data[:ntrain, ::r1][:, :s1].reshape(ntrain, s1, 1).repeat(1, 1, s2)
    x_train = x_train.reshape(ntrain, -1, 1)
    y_train = output_data[:ntrain, ::r1, ::r2][:, :s1, :s2]
    y_train = y_train.reshape(ntrain, -1, Deformation, T)
    x_test = input_data[-ntest:, ::r1][:, :s1].reshape(ntest, s1, 1).repeat(1, 1, s2)
    x_test = x_test.reshape(ntest, -1, 1)
    y_test = output_data[-ntest:, ::r1, ::r2][:, :s1, :s2]
